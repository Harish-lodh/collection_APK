...
export default function LoanDetailsScreen() {
  ...
  const handleSearch = async () => {
    ...
    if (String(selectedProduct).toLowerCase().trim() === 'embifi') {
      apiUrl = `${BACKEND_BASE_URL}/embifi/user-Details`;
    } else if (String(selectedProduct).toLowerCase().trim() === 'malhotra') {
      apiUrl = 'https://fintreelms.com/api/collection/malhotra/search';
    } else {
      alert('Unsupported product selected');
      setIsLoading(false);
      return;
    }
